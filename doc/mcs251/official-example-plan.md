...
状态：MT-4A 来源记录（已完成）。当前相关实现处于 `CONTINUE MT-4B`；
本计划本身不是法律意见或发布许可；真实项目样本的用户确认范围由样例目录的
`AUTHORIZATION.md` 单独记录。
...
仓内原创文件可以作为开发种子，但在来源门禁尚未满足时不能替代“至少一组拥有合法
使用权的真实 C251/8051 项目”这一 MT-4A 关闭输入；该输入现已由
`stc_diyclock` 的确认记录满足。公开网页可供阅读和事实核对，网页上的“可下载”或
“官方”字样本身不等于允许复制或再分发实现代码。
...
业务逻辑。其预期值由文件内常量和纯 C 运算独立确定。它已作为 MT-4A 的矩阵样本，
并供当前 MT-4B 诊断回归复用；不能被重新标注为“从真实项目提取”，也不能单独关闭
真实项目来源门禁。

### 对该仓库的当前判定（MT-4A 历史候选评估，2026-09-01）

该仓库曾作为 MT-4A 的真实项目候选和语义线索：项目目录覆盖流水灯、PWM、I2C/SPI、
...
该候选在进入任何仓内迁移样本来源前，需要作者或相应权利人确认：具体目录/提交、
源文件权利归属、允许抽取的最小范围、允许将独立重写 fixture 提交到公共 SDCC 仓库的
范围，以及第三方库和厂商文件的排除范围。在确认前，本项目只保留上述公开元数据记录，
不提取该仓库源码，不分析其中的 Keil/厂商
头文件、库、startup、对象、Listings 或二进制。

### `zerog2k/stc_diyclock` 的最小派生边界

该仓库是一个已归档的 STC15 DIY 时钟固件项目，README 公开说明使用 SDCC 和
传统 `make` 构建。固定的 `main` revision、上游 `LICENCE` 和本地保留文本见
`support/stc32/tests/migration/cases/project/stc_diyclock/`。其 `src/main.c`
包含与本矩阵直接相关的 `__bit`、`__interrupt 1 __using 1` 和 inline assembly
形态；本阶段只以其中的前两类源代码形态制作最小派生 fixture。

根目录 MIT 许可是该仓库声明的许可基础，但不自动覆盖仓库中所有文件。尤其
...
- `stc_diyclock` 已补录项目标识、固定 revision、许可证文本、排除范围、抽取规则、
  用户确认和对应矩阵行。2026-09-01 的确认闭合真实样本来源输入；MT-4A 已由项目
  主审关闭，当前执行状态为 `CONTINUE MT-4B`，MT-4B 是否关闭仍由项目主审结合测试、
  CI 和其余证据裁决。
