...
cl_uc251::exec_0b(t_mem sub, int dec)
{
  ...
  if (kind == 2 && reg == 15)
    ...
    set_dr(reg * 4, dec ? get_dr(reg * 4) - step : get_dr(reg * 4) + step);
  else if (kind == 1)
    set_wr(reg * 2, dec ? get_wr(reg * 2) - step : get_wr(reg * 2) + step);
  else
    set_r8(reg, dec ? get_r8(reg) - step : get_r8(reg) + step);
  return(resGO);
}
